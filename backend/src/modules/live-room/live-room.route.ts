...
router.post(
  ...
  auth("ADMIN", "USER"),
  uploadRoomImage,
  LiveRoomControllers.uploadImage,
);
...
router.get(
  "/:id/messages",
  auth("ADMIN", "USER"),
  LiveRoomControllers.getMessages,
);

// List all banned users for a specific room.
router.get(
  "/:roomName/banned-users",
  auth("ADMIN", "USER"),
  LiveRoomControllers.getBannedUsers,
);
